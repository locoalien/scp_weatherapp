...
import com.sap.hana.cloud.samples.weather.model.FavoriteCity;
...
public class FavoriteCityService 
{
	...
	public List<FavoriteCity> getFavoriteCities(@Context SecurityContext ctx)
	{
		...
		//Obteniendo el ID del usuario para proceder a listar las ciudades favoritas.
	...
	@DELETE
	@Path("/{id}")
	public List<FavoriteCity> removeFavoriteCity(@Context SecurityContext ctx,@PathParam(value = "id") String id)
	{
		List<FavoriteCity> retVal = null;
		
		//Solo elimina las ciudades favoritas del usuario logueado
		String userName = (ctx.getUserPrincipal() != null) ? ctx.getUserPrincipal().getName() : "anonymous";
		Map<String,String> props = new HashMap<String,String>();
		props.put("tenant.id", userName);
